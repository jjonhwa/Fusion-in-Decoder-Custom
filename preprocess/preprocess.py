...
def del_except(data: pd.DataFrame):
    """
        Summarization 수행 시 생긴 '결과 없음' 데이터를 제외
    """
    count = 0

    no_result_idx = []
    if 'ko_wikipedia_list' in data.columns:
        for i, ko_wiki in enumerate(data['ko_wikipedia_list']):
            if ko_wiki == '결과 없음':
                count += 1
                no_result_idx.append(i)
        
        data.drop(no_result_idx, axis=0, inplace=True)
        data.reset_index(drop=True, inplace=True)
    
    return data, count

def summary_parse(data: pd.DataFrame):
    """
        Ko_wikipedia dataset 활용할 경우 적용

        Return: | Context | Summary |
    """
    summary = []
    for ko_wiki in data['ko_wikipedia_list']:
        summary.append((" ".join(ko_wiki.split("\n")[1:])).strip().split("Summary:")[1].strip())
    
    return data['context'].tolist(), summary

def preprocess_length(
        context: List, 
        summary: List,
        tokenizer,
        context_min_length: int = 0,
        context_max_length: int = 1_000_000,
        summary_min_length: int = 0,
        summary_max_length: int = 1_000_000,
):
    """
        필요에 따라, 길이 제한
    """
    preprocessed_index = []
    for i, (c, s) in tqdm(enumerate(zip(context, summary))):
        tokenized_context = tokenizer.encode(c)
        tokenized_summary = tokenizer.encode(s)

        if (context_min_length <= len(tokenized_context) <= context_max_length) or \
                (summary_min_length <= len(tokenized_summary) <= summary_max_length):
            preprocessed_index.append(i)

    final_context = [c for i, c in enumerate(context) if i in preprocessed_index]
    final_summary = [s for i, s in enumerate(summary) if i in preprocessed_index]

    return final_context, final_summary