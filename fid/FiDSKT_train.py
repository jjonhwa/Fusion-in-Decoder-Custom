...
from src.modeling_encoder_decoder import EncoderDecoderModel
from src.configuration_encoder_decoder import EncoderDecoderConfig
    ...
    while True:
        ...
        for i, batch in tqdm(enumerate(train_dataloader)):
            ...
            if step % opt.eval_freq == 0:
                ...
                dev_score, new_df = evaluate(model, eval_dataset, tokenizer_output, collator, opt)
...
def get_dataset(opt):
    ...
    train_data = new_data.sample(round(len(new_data) * 0.99), random_state=42)
...
if __name__ == '__main__':
    ...
    model.load_encdec(encoder_decoder.state_dict())
    # model = model.to(opt.device)

    del encoder_decoder
    torch.cuda.empty_cache()

    model.encoder.config = model.encoder.encoder.config
    model.decoder.config = model.decoder.decoder.config

    model.config.decoder_start_token_id = tokenizer_output.bos_token_id
    model.config.pad_token_id = tokenizer_output.pad_token_id
    model.config.vocab_size = model.config.decoder.vocab_size

    decoder_block_count = len(model.decoder.decoder.transformer.h)
    for i in range(decoder_block_count):
        model.decoder.decoder.transformer.h[i].is_decoder = True

    # GQA는 적용하지 않음
    if opt.kv_heads == -1 and opt.n_cross_layer == -1: # w/o GQA, LSA
        print("Original")
        pass
    elif opt.kv_heads == -1: # w LSA
        print("with LSA")
        # model = src.model.convert_t5_to_fido(model, 
        #                                      kv_heads=t5.config.num_heads)
        model = src.model.convert_LSA(model,
                                      n_cross_layer=opt.n_cross_layer,
                                      block = 'skt')
    ...
    # opt.per_gpu_batch_size = 2 # 이 배치의 크기만큼 나뉘어 가져간 것으로 보임
    # opt.eval_freq = 1000
